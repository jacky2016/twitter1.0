define(['./jquery'], function ($) {
    $.fn.expression = function (o) {
        ...
        This.html($[FO]('<div class="divExpression"><div class="divTable"></div><div class="result" result=""></div>{0}<input type="button" value="确定" class="btnOK" /><input type="button" value="取消" class="btnCancel" /></div>', multParent));
                  ...
                  expression = {
                      ...
                      DrawDefaultNode: function () {
                          ...
                          $('.btnOK', This)[CK](function () {
                              if (!isPass) { alert(passTxt); return; }
                              if (o.onRegExp() === FE) { alert('表达式验证错误!'); return; }
                              t.PerfixExpress();
                              t.GetExpText(expObj);
                              t.GetExpKey(expObj);
