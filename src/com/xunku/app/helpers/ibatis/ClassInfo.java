...
public class ClassInfo {
	...
	private static final Map CLASS_INFO_MAP = Collections
	...
	private ClassInfo(Class clazz) {
		...
		addSetMethods(clazz);
		addFields(clazz);
		readablePropertyNames = (String[]) getMethods.keySet().toArray(
				new String[getMethods.keySet().size()]);
		writeablePropertyNames = (String[]) setMethods.keySet().toArray(
	...
	private void addSetterConflict(Map conflictingSetters, String name,
			Method method) {
	...
	private void resolveSetterConflicts(Map conflictingSetters) {
		for (Iterator propNames = conflictingSetters.keySet().iterator(); propNames
				.hasNext();) {
	...
	public Object instantiateClass() {
		if (defaultConstructor != null) {
			try {
				...
				throw new RuntimeException("Error instantiating class. Cause: "
						+ e, e);
			}
		} else {
			throw new RuntimeException(
					"Error instantiating class.  There is no default constructor for class "
							+ className);
		}
	}
	...
	public static Throwable unwrapThrowable(Throwable t) {
		...
		while (true) {
			if (t2 instanceof InvocationTargetException) {
				...
				t2 = ((UndeclaredThrowableException) t)
